...
select d.doc_id, d.doc_name, d.doc_birth, p.dep_no, d.doc_name 
from doctors d join departments p using(dep_no);
...
#nurse, doctor dep_no
select n.nur_id, n.nur_name, n.nur_birth, d.dep_no, d.doc_name
from nurses n join doctors d using(dep_no);
...
#depart dep_no, /doctor doc_id, dep_no, /patients nur,pat_id, doc_id,/ nurses nur_id, dep_no
#chart treat_no, chart,doc, pat_id/ treate treat_no ,doc,pat_id
select p.pat_id, p.pat_name, p.pat_jumin, p.pat_phone, d.doc_name, n.nur_name
from doctors d join patients p using(doc_id)
join nurses n on n.dep_no=p.nur_id;
...
#문제4 모든 진료내역에서 환자 이름, 담당의사 이름, 트리트-진료내용, 차트-처방내용, t-진료날짜를 조회하시오.
select p.pat_name, d.doc_name, t.treat_contents, c.chart_contents, t.treat_datetime
from patients p 
join doctors d using(doc_id) 
-- join charts c on c.doc_id=d.doc_id
join charts c on c.doc_id=t.doc_id
join treatments t on t.pat_id=p.pat_id;
...
select m.dep_no, p.pat_name, d.doc_name, t.treat_contents, c.chart_contents, t.treat_datetime
from departments m 
join doctors d on m.dep_no= d.dep_no
join patients p on d.doc_id=p.doc_id
join charts c on p.doc_id=c.doc_id
join treatments t on c.doc_id=t.doc_id
where m.dep_name='외과';
...
select*from patients
 where (year(now())-concat('19', left('pat_jumin',2))>=30  #left()문자열의 왼쪽에서 2개의 
 #문자를 추출하는 함수
 and year(now())-concat('19', left('pat_jumin',2))> 40);
...
select d.doc_name, d.dap_name from departments d left join doctors t
on d.dep_no=t.dep_no where t.doc_id is null;
...
select u.nur_id, n.nur_name, count(*)
from nurses n join patients p
on n.nur_id=p.nur_id
group by n.nur_id
order by count(*) desc
limit 1;
